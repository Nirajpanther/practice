  ...
  const deletePermission = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        apiPermission.deletePermission(id).then((res) => props.data(res));
  ...
  return (
    <div className="card">
      <div className="card-header row align-items-center m-0">
        ...
        {props.perms?.indexOf("add permission") != -1 && <AddPermission />}
      </div>
          ...
          <tbody>
            {permission.map((items, i) => (
              <tr key={i}>
                ...
                <td> {items.name} </td>
                <td>
                  {props.perms?.indexOf("edit permission") != -1 && (
                    <Edit id={items.id} />
                  )}{" "}
                  {props.perms?.indexOf("delete permission") != -1 && (
                    <Delete id={items.id} data={(res) => setPermission(res)} />
                  )}
                </td>
              </tr>
            ))}
...
function Permissionlist() {
  ...
  return (
    <>
      {/* {console.log(userPermissions).indexOf("view permission")} */}
